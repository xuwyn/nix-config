...
This flake uses [Attic](https://docs.attic.rs/tutorial.html) as a self-host cache server. This section documents
most of the steps I'd likely forget if I have to start this from scratch.
...
The traffic is routed via tailscale and LAN with `nginx` acts as reverse proxies for both
...
Import `<class>.attic` into the corresponding configurations. The default atticd server is `puffin` for both
`tailscaleDomain` and `lanDomain`, override them in individual configuration or set either of them to `null` to disable that domain.
...
The clients have to trust the self-signed cert generated by the atticd-hosted server to use LAN (masked by mDNS) as an
alternative to tailscale.

This can be done easily on NixOS by adding the cert (can be found at `/var/lib/atticd-certs/lan.crt`
on the server) to `security.pki.certificateFiles`.

MacOS security makes it so that the most painless way to add a self-signed cert is to do it manually 😑

```sh
# add new crt
sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ./path/to/lan.crt

# check new crt
security find-certificate -c "hostname.local" /Library/Keychains/System.keychain

# remove expired crt
sudo security delete-certificate -c "hostname.local" /Library/Keychains/System.keychain
```

For Arch-based distros:

```sh
sudo cp ./path/to/lan.crt /etc/ca-certificates/trust-source/anchors/atticd.crt
...
```sh
sudo cp ./path/to/lan.crt /usr/local/share/ca-certificates/atticd.crt
...
https://hostname.local/main/nix-cache-info
