{
  ...
  defaultTerm = (import ../../../hosts/${host}/variables.nix).terminal;
in {
  home.packages = [
    (pkgs.writeShellScriptBin
      ...
      ''
        ...
        # Behavior:
        #   - If <terminal_command> is omitted, uses $TERM, with fallback to default when $TERM is unset/empty
        ...
        # Terminal command: Use default if no terminal is set
        TERMINAL_CMD="''${1-}"
        if [ -z "''${TERMINAL_CMD}" ]; then
          TERMINAL_CMD=${defaultTerm}
        fi
        ...
        spawn_terminal() {
          ...
          debug_echo "Failed to detect new window; aborting spawn"
          return 1
        }
